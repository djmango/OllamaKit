...
public class OllamaKit {
    ...
    public var restartInterval: TimeInterval = 20
...
public extension OllamaKit {
    ...
    /// Waits for the API to become reachable.
    func waitForAPI(timeoutSeconds: Int = 10) async throws {
        ...
        logger.error("Failed to reach API within \(timeoutSeconds) seconds.")
...
extension OllamaKit {
    ...
    private func shouldRestartInference(data: OKChatRequestData? = nil) -> Bool {
        let ourself = self
        if OllamaKit.shared.lastInferenceTime < Date.now.addingTimeInterval(-ourself.restartInterval) {
            logger.debug("Restarting Ollama because it has been more than \(ourself.restartInterval) seconds since the last message.")
            return true
...
public extension OllamaKit {
        ...
        let waitForAPIFuture = Future<Void, Error> { promise in
            Task {
                do {
                    ...
                    if self.shouldRestartInference(data: data) {
                        self.restart()
                    }
                    try await self.waitForAPI()
                    promise(.success(()))
                } catch {
    ...
    /// This method is a wrapper around the chat method that allows for the use of async/await
    func achat(data: OKChatRequestData) async throws -> OKChatResponse {
    ...
    internal func extractNextJSONObject(from buffer: inout Data) -> Data? {
        var depth = 0
